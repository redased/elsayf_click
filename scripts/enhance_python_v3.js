...
> [!IMPORTANT]
> Ne poussez jamais vos secrets (clés API) sur Git !
`
    }
};
...
async function main() {
    ...
    console.log('--- Final Comprehensive Enhancement ---');
    ...
    for (const [title, data] of Object.entries(lessonData)) {
        ...
        if (!lesson) {
            ...
            await prisma.lesson.update({
                where: { id: lesson.id },
                data: {
                    content: data.intro // Reset/Update intro
                }
            });
            console.log(`[UPDATE] Lesson updated: ${title}`);
        }

        const textContent = await prisma.courseContent.findFirst({
    ...
    console.log('✅ Success! Course "Python Integral" now has 20 highly detailed lessons.');
}
