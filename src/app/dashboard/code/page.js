...
export default function CodePage() {
    ...
    useEffect(() => {
        const checkAccess = async () => {
            ...
            try {
                // Check if user has VSCode access
                const response = await fetch('/api/user/access-check');
                const data = await response.json();

                // Autoriser si vscodeAccess=true OU si SUPER_ADMIN
                if (data.vscodeAccess || session.user.role === 'SUPER_ADMIN') {
                    setHasAccess(true);
                } else {
                    setHasAccess(false);
                }
            ...
            <div className="fixed inset-0 bg-[#1e1e1e] flex items-center justify-center">
                <div className="text-center max-w-md">
                    ...
                    <p className="text-gray-400 mb-6">
                        Vous n'avez pas accès à l'IDE VS Code.
