...
function CVBuilderInner() {
  ...
  // Mapping template URL → preset correspondant (clés réelles de defaultPresets.js)
  const TEMPLATE_TO_PRESET = {
    'modern-tech': 'developer',
    'executive-rh': 'rh',
    'creative-designer': 'designer',
    'minimalist': 'data_analyst',
    'dual-column': 'business_finance',
    'emerald': 'rh',
  };
  
  const defaultPresetKey = (urlTemplate && TEMPLATE_TO_PRESET[urlTemplate]) ? TEMPLATE_TO_PRESET[urlTemplate] : 'developer';
  const defaultPreset = PRESET_PROFILES[defaultPresetKey] || PRESET_PROFILES.developer;

  const [data, setData] = useState(defaultPreset.data);
  const [config, setConfig] = useState({
    template: urlTemplate || defaultPreset.template,
    color: defaultPreset.color,
    font: defaultPreset.font,
    ...
    <Suspense fallback={
      <div className="min-h-screen bg-[#050814] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-violet-600 to-pink-500 flex items-center justify-center text-white font-black text-lg animate-pulse">CV</div>
          <p className="text-gray-400 text-sm font-medium">Chargement du studio CV...</p>
        </div>
      </div>
    }>
      <CVBuilderInner />
